"""Audio plugin for music and sound effects.
...
Audio Organization:
    The audio plugin expects files to be organized in the assets/audio/ directory:
...
Integration:
    - PlayMusicAction and PlaySFXAction in the scripting plugin
...
class AudioPlugin(AudioBasePlugin):
    """Manages background music and sound effects.

    The AudioPlugin is the central plugin for all audio playback in the game.
    It provides:
    ...
    def cleanup(self) -> None:
        ...
        logger.debug("AudioPlugin cleanup complete")

    def reset(self) -> None:
        """Reset audio plugin for new game (stop music, keep cache)."""
        self.stop_music()
        logger.debug("AudioPlugin reset complete")
    ...
    def get_save_state(self) -> dict[str, Any]:
        ...
        This implements the BasePlugin interface. For backwards compatibility,
    ...
    def on_scene_loaded(self, scene_name: str) -> None:
        """Play background music configured for the loaded scene.
        ...
        if not music_filename:
            logger.debug("No 'music' configured for map '%s'", scene_name)
            return

        logger.info("Loading music for map '%s': %s", scene_name, music_filename)
