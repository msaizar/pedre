...
- Integration with game scripting plugin via animation events
...
class AnimatedNPC(AnimatedSprite):
    ...
    def __init__(
        ...
        *,
        ...
        disappear_frames: int | None = None,
        disappear_row: int | None = None,
        ...
        Args:
            ...
            NPC-specific special animations:
            appear_frames: Number of frames for appear animation. If only this is set,
                disappear will be auto-generated by reversing.
            appear_row: Row index for appear animation frames.
            disappear_frames: Number of frames for disappear animation. If only this is set,
                appear will be auto-generated by reversing.
            disappear_row: Row index for disappear animation frames.
        ...
        self.disappear_row = disappear_row
    ...
    def _load_npc_textures(self, sprite_sheet_path: Path | str) -> None:
        ...
        # Load appear/disappear animations with auto-generation
        if self.appear_frames is not None and self.appear_row is not None:
            if self.disappear_frames is not None and self.disappear_row is not None:
                # Case 1: Both defined - load both independently
                appear_frames = self.appear_frames
                appear_row = self.appear_row
                disappear_frames = self.disappear_frames
                disappear_row = self.disappear_row

                logger.debug("Loading appear (%s frames, row %s)", appear_frames, appear_row)
                logger.debug("Loading disappear (%s frames, row %s)", disappear_frames, disappear_row)

                # Load appear in reverse (invisible → visible)
                for frame_num in range(appear_frames - 1, -1, -1):
                    left = frame_num * self.tile_size
                    top = appear_row * self.tile_size
                    right = left + self.tile_size
                    bottom = top + self.tile_size
                    frame_image = sprite_sheet.crop((left, top, right, bottom))
                    texture = arcade.Texture(
                        name=f"npc_appear_{appear_frames - 1 - frame_num}",
                        image=frame_image,
                    )
                    self.animation_textures["appear"].append(texture)

                # Load disappear in forward (visible → invisible)
                for frame_num in range(disappear_frames):
                    left = frame_num * self.tile_size
                    top = disappear_row * self.tile_size
                    right = left + self.tile_size
                    bottom = top + self.tile_size
                    frame_image = sprite_sheet.crop((left, top, right, bottom))
                    texture = arcade.Texture(
                ...
                # Case 2: Only appear defined - load appear and generate disappear
                appear_frames = self.appear_frames
                appear_row = self.appear_row

                logger.debug("Loading appear (%s frames, row %s)", appear_frames, appear_row)
                logger.debug("Auto-generating disappear by reversing appear")

                # Load appear in reverse
                for frame_num in range(appear_frames - 1, -1, -1):
                    left = frame_num * self.tile_size
                    top = appear_row * self.tile_size
                    right = left + self.tile_size
                ...
                # Generate disappear by reversing appear
                self.animation_textures["disappear"] = list(reversed(self.animation_textures["appear"]))

        elif self.disappear_frames is not None and self.disappear_row is not None:
            # Case 3: Only disappear defined - load disappear and generate appear
            disappear_frames = self.disappear_frames
            disappear_row = self.disappear_row

            logger.debug("Loading disappear (%s frames, row %s)", disappear_frames, disappear_row)
            logger.debug("Auto-generating appear by reversing disappear")

            # Load disappear in forward
            for frame_num in range(disappear_frames):
                left = frame_num * self.tile_size
                top = disappear_row * self.tile_size
                right = left + self.tile_size
        ...
        # Load interact left/right with auto-generation logic using parent's helper
