...
class TestAudioPlugin(unittest.TestCase):
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_cleanup(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        self.plugin.play_music("test.ogg")
        self.plugin.play_sfx("test.mp3")

        # Verify state before cleanup
        assert self.plugin.current_music is not None
        assert self.plugin.music_player is not None
        assert len(self.plugin.music_cache) > 0
        ...
        assert self.plugin.music_player is None
        assert len(self.plugin.music_cache) == 0
        assert len(self.plugin.sfx_cache) == 0

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_reset(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Add to cache
        cache_size_before = len(self.plugin.music_cache)
        ...
        mock_player.pause.assert_called_once()
        assert self.plugin.music_player is None
        assert len(self.plugin.music_cache) == cache_size_before

    # Test cache getter and setter
    def test_get_music_cache(self) -> None:
        """Test getting music cache."""
        mock_sound = MagicMock()
        self.plugin.music_cache["test.ogg"] = mock_sound

        cache = self.plugin.get_music_cache()

        assert "test.ogg" in cache
        assert cache["test.ogg"] == mock_sound

    def test_set_music_cache(self) -> None:
        """Test setting music cache."""
        mock_sound = MagicMock()

        self.plugin.set_music_cache("test.ogg", mock_sound)

        assert "test.ogg" in self.plugin.music_cache
        assert self.plugin.music_cache["test.ogg"] == mock_sound

    # Test play_music with caching
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_music_uses_cache(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_music_streaming_not_cached(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        self.plugin.play_music("test.ogg", loop=False)
        ...
        # Verify not cached
        assert "test.ogg" not in self.plugin.music_cache

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_music_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        result = self.plugin.play_music("test.ogg")
    ...
    def test_play_music_waits_for_background_preload(
        self, mock_asset_path: Mock, mock_load_sound: Mock, mock_sleep: Mock
    ) -> None:
        ...
        self.plugin._music_loading.add("test.ogg")
        ...
        def complete_preload(*_args: object, **_kwargs: object) -> None:
            ...
            if call_count >= 3:
                ...
                self.plugin._music_loading.discard("test.ogg")
                self.plugin.music_cache["test.ogg"] = mock_sound
        ...
        assert "test.ogg" not in self.plugin._music_loading  # Should be cleared
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_stop_music(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        """Test stopping music."""
        # Set up music playing
        mock_sound = MagicMock()
        mock_player = MagicMock()
        ...
        # Stop music
        self.plugin.stop_music()

        mock_player.pause.assert_called_once()
        assert self.plugin.music_player is None

    def test_stop_music_with_no_player(self) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_stop_music_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_pause_music(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        """Test pausing music."""
        # Set up music playing
        mock_sound = MagicMock()
        mock_player = MagicMock()
        ...
        # Pause music
        self.plugin.pause_music()

        mock_player.pause.assert_called()

    def test_pause_music_with_no_player(self) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_pause_music_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_resume_music(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        """Test resuming music."""
        # Set up music playing
        mock_sound = MagicMock()
        mock_player = MagicMock()
        ...
        # Resume music
        self.plugin.resume_music()

        mock_player.play.assert_called_once()

    def test_resume_music_with_no_player(self) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_resume_music_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_set_music_volume(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Set volume
        self.plugin.set_music_volume(0.8)

        assert self.plugin.music_volume == 0.8
    ...
    def test_set_music_volume_clamping(self) -> None:
        ...
        # Test upper bound
        self.plugin.set_music_volume(1.5)
        assert self.plugin.music_volume == 1.0

        # Test lower bound
        self.plugin.set_music_volume(-0.5)
        assert self.plugin.music_volume == 0.0

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_set_music_volume_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_sfx(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Play SFX
        result = self.plugin.play_sfx("test.mp3")
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_sfx_with_custom_volume(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        result = self.plugin.play_sfx("test.mp3", volume=0.3)
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_sfx_caching(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    def test_play_sfx_when_disabled(self) -> None:
        """Test that SFX doesn't play when disabled."""
        self.plugin.sfx_enabled = False
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_sfx_file_not_found(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Should return False and not raise
        result = self.plugin.play_sfx("missing.mp3")

        assert result is False

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_play_sfx_exception_handling(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    def test_set_sfx_volume(self) -> None:
        ...
        self.plugin.set_sfx_volume(0.8)

        assert self.plugin.sfx_volume == 0.8

    def test_set_sfx_volume_clamping(self) -> None:
        """Test that SFX volume is clamped to valid range."""
        # Test upper bound
        self.plugin.set_sfx_volume(1.5)
        assert self.plugin.sfx_volume == 1.0

        # Test lower bound
        self.plugin.set_sfx_volume(-0.5)
        assert self.plugin.sfx_volume == 0.0

    # Test toggle functions
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_toggle_music(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        self.plugin.play_music("test.ogg")
        assert self.plugin.music_enabled is True

        # Toggle off
        result = self.plugin.toggle_music()

        assert result is False
        assert self.plugin.music_enabled is False
        mock_player.pause.assert_called_once()
    ...
    def test_toggle_sfx(self) -> None:
        ...
        assert self.plugin.sfx_enabled is True

        # Toggle off
        result = self.plugin.toggle_sfx()

        assert result is False
        assert self.plugin.sfx_enabled is False
    ...
    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_clear_sfx_cache(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Clear cache
        self.plugin.clear_sfx_cache()

        assert len(self.plugin.sfx_cache) == 0

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_clear_music_cache(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
        ...
        # Clear cache
        self.plugin.clear_music_cache()

        assert len(self.plugin.music_cache) == 0

    @patch("pedre.plugins.audio.plugin.arcade.load_sound")
    @patch("pedre.plugins.audio.plugin.asset_path")
    def test_clear_all_caches(self, mock_asset_path: Mock, mock_load_sound: Mock) -> None:
    ...
    def test_from_dict(self) -> None:
        ...
        assert self.plugin.music_volume == 0.3
    ...
    def test_from_dict_partial(self) -> None:
        ...
        assert self.plugin.music_volume == 0.3
        assert self.plugin.sfx_volume == 0.7  # Unchanged
    ...
    def test_get_save_state(self) -> None:
        ...
        assert state == self.plugin.to_dict()
    ...
    def test_load_from_tiled_with_non_string_music(self) -> None:
